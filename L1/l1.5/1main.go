/* ВАРИАНТ №1 - решение с применением time.After */
...
func main() {
	...
	// засекаем время.
	// инициализация timeLimit вынесена за пределы цикла с select,
	// чтобы при каждой итерации не получать новый таймер
	timeLimit := time.After(n * time.Second)
...
loop:
	for {
		...
		case ch <- number: // если можем, отправляем число в канал ch
			number++
		case <-timeLimit: // когда приходит время - выходим из цикла
